...
while(loop<=no_of_rows)
    currentclass=hayesroth(loop,1:4);
    charval=hayesroth{loop,5};           %Check whether it is 1,2 or 3
    low=loop-1;
    high=loop+1;
    if(loop==1)
        trainingclass=hayesroth(2:no_of_rows,1:5);
        
    elseif(loop==no_of_rows)
        trainingclass=hayesroth(1:no_of_rows-1,1:5);
        
    else
        trainingclass1=hayesroth(1:low,1:5);
        trainingclass2=hayesroth(high:no_of_rows,1:5);
    ...
    while(j<=no_of_rows-1)    %for checking for R
        if(trainingclass{j,5}==3 && trainingclass{j,1}==data1)
            p1byR=p1byR+1;
        end
        if(trainingclass{j,5}==3 && trainingclass{j,2}==data2)
            p2byR=p2byR+1;
        end
        if(trainingclass{j,5}==3 && trainingclass{j,3}==data3)
            p3byR=p3byR+1;
        end
        if(trainingclass{j,5}==3 && trainingclass{j,4}==data4)
            p4byR=p4byR+1;
        end
        j=j+1;
    end
