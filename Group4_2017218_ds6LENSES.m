...
while(loop<=no_of_rows)
    currentclass=lenses(loop,1:4);
    charval=lenses{loop,5};           %Check whether it is 1,2 or 3
    low=loop-1;
    high=loop+1;
    if(loop==1)
        trainingclass=lenses(2:no_of_rows,1:5);
    
    elseif(loop==no_of_rows)
        trainingclass=lenses(1:no_of_rows-1,1:5);
        
    else
        trainingclass1=lenses(1:low,1:5);              
        trainingclass2=lenses(high:no_of_rows,1:5);
